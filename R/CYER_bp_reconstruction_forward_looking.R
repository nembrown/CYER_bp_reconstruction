...
creel_full_catch_chinook<- creel_full_catch_chinook %>% rename(AREA_NUM = AREA, AREA = AREA_GROUP, CREEL=VAL, DISPOSITION = TYPE)
names(creel_full_catch_chinook)<- tolower(names(creel_full_catch_chinook))
...
gst<-c("Area 13", "Area 13 SoG", "Area 14", "Area 15", "Area 16", "Area 17", "Area 18", "Area 19", "Area 19 (GS)", "Area 28", "Area 29") 
jst<-c("Area 11", "Area 111", "Area 12")
jdf<-c("Area 19", "Area 19 (JDF)", "Area 20", "Area 20 (East)", "Area 20 (West)")


creel_full_catch_chinook<- creel_full_catch_chinook %>%  mutate(erafishery = case_when(
...
creel_full_catch_chinook_area_erafishery_month<- creel_full_catch_chinook %>%  group_by(area, erafishery, year, month, disposition) %>% 
                                                         summarise(creel = sum(creel, na.rm=TRUE))  


creel_full_catch_chinook_area_erafishery_year<- creel_full_catch_chinook %>%  group_by(area, erafishery, year, disposition) %>% 
  summarise(creel = sum(creel, na.rm=TRUE))  
...
creel_kris_dupes <-  creel_kris %>% get_dupes(- c(ESTIMATE_SOURCE, LINK, ESTIMATE, STANDARD_ERROR)) %>%  as_tibble() %>% arrange(ESTIMATE)
write.csv(creel_kris_dupes, "creel_kris_dupes.csv")

creel_kris_wide<- creel_kris %>% select(-LINK, -STANDARD_ERROR) %>% pivot_wider(names_from = ESTIMATE_SOURCE, values_from = ESTIMATE) %>% 
                                 mutate(flag = case_when(
                                   !is.na(Creel) & !is.na(`In Fill`) ~ "yes", 
                                   TRUE ~ "no"
                                 ))
View(creel_kris_wide)
write.csv(creel_kris_wide, "creel_kris_wide.csv")


creel_nick_dupes <-  creel_nick %>% get_dupes(- c(DATASOURCE, SOURCE, VAL, VALTYPE, DESCRIPTION, STATUS)) %>%  as_tibble() 
write.csv(creel_nick_dupes, "creel_nick_dupes.csv")
...
creelcc <- creel_kris %>%
  ...
  select(AREA, YEAR, MONTH, TYPE, ESTIMATE_SOURCE, ESTIMATE, STANDARD_ERROR) %>% 
